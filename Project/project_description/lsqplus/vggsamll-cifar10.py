...
import datetime
from utils.utils_preprocess import CenterPaste
...
def trainer():
    ...
    config = {'a_bit':4, 'w_bit':4, "all_positive":False, "per_channel":True, 
              "num_classes":10,"batch_init":20}
    pretrainedmodel = r'./log/model_1.pth'
    ...
    per_pixel_mean = torch.tensor(np.mean(trainset.data, axis=0).reshape(3, 32, 32), dtype=torch.float32)


    def ppm(x):
        return (x * 255.0 - per_pixel_mean) / 128.0
    ...
    time.sleep(3)
    adam = False
    lr = 0.02 # initial learning rate (SGD=1E-2, Adam=1E-3)
    momnetum=0.9
        ...
        if epoch%3==0 and epoch>nowepoch:
            ...
            try:
                if epoch>nowepoch and Accuracy>pre:
                    torch.save(savestate, os.path.join(savepath, prefix+'_models_vggsmall_{}_{}_{}_{:.3f}_{}_{}.pth'.format(

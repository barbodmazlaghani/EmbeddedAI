...
def trainer():
    ...
    config = {'a_bit':5, 'w_bit':5, "all_positive":False, "per_channel":True, 
              "num_classes":10,"batch_init":21}
    ...
    time.sleep(3)
    adam = False
    lr = 0.1 # initial learning rate (SGD=1E-2, Adam=1E-3)
    momnetum=0.9
    params = [p for p in model.parameters() if p.requires_grad]
    # if adam:
    #     optimizer = optim.Adam(params, lr=lr, betas=(momnetum, 0.999))  # adjust beta1 to momentum
    # else:
    optimizer = optim.SGD(params, lr=lr, momentum=momnetum, weight_decay=1e-4)
